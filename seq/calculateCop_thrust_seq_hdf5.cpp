...
#include <hdf5.h> // HDF5 output support
...
// -----------------------------
int main() {
    NetCDFFile precFile, tgFile;
    HDF5Out outH5; // HDF5 output
    int retval;
    double time[TIME];
    initNetCDFandHDF5(&precFile, &tgFile, &outH5, time);
    ...
    hsize_t write_count[3] = { 1, 1, TIME };
    ...
    for (int j = 0; j < LAT; j++) {
      for (int k = 0; k < LON; k++) {
        ...
        H5_ERR(H5Sselect_hyperslab(filespace, H5S_SELECT_SET,
                            write_start, NULL, write_count, NULL));
